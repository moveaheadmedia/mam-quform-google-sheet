...
use QuformToGoogleSheet\Common\Traits\Requester;
use QuformToGoogleSheet\Common\Utils\Errors;
use QuformToGoogleSheet\Config\Classes;
use QuformToGoogleSheet\Config\I18n;
use QuformToGoogleSheet\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'quform_to_google_sheet_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'quform_to_google_sheet_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'quform-to-google-sheet' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'Quform To Google Sheet classes are not being loaded by Composer\'s Autoloader' ),
